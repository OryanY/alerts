    ...
    return (
      <div style={{
        ...
        <div style={{textAlign: 'center'}}>
          ...
          <div style={{fontSize: 18, color: '#475569', fontWeight: 500}}>Loading rules...</div>
        </div>
      </div>
    );
  }
    ...
    <div style={{maxWidth: '100%', margin: '0 auto'}}>
      {error && (
        <div style={{
          ...
          <button onClick={() => setError(null)} style={{marginLeft: 'auto', background: 'none', border: 'none', color: '#dc2626', cursor: 'pointer', padding: 4}}>
            <X size={16} />
          </button>
        </div>
      )}
      ...
      <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 32, padding: '0 8px'}}>
        ...
        <div style={{display: 'flex', gap: 12}}>
          <button
            ...
            style={{
              background: showForm ? 'linear-gradient(135deg, #f59e0b 0%, #d97706 100%)' : 'linear-gradient(135deg, #7c3aed 0%, #a855f7 100%)',
              color: 'white',
              border: 'none',
              borderRadius: 12,
              padding: '12px 24px',
          ...
          <button onClick={fetchRules} style={{background: 'white', color: '#475569', border: '2px solid #e2e8f0', borderRadius: 12, padding: '12px 20px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 8, fontSize: 14, fontWeight: 500}}>
            <RefreshCw size={16} />
            Refresh
          </button>
        </div>
      ...
      {showForm && (
        <div id="rule-form" style={{background: 'white', borderRadius: 16, padding: 32, marginBottom: 32, boxShadow: '0 20px 25px -5px rgba(0, 0, 0, 0.1)', border: '1px solid #f1f5f9', position: 'relative', overflow: 'hidden'}}>
          <div style={{position: 'absolute', top: 0, left: 0, right: 0, height: 6, background: 'linear-gradient(90deg, #7c3aed, #a855f7, #c084fc)', borderRadius: '16px 16px 0 0'}} />

          <div style={{marginTop: 8}}>
            <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24}}>
              <div>
                <h3 style={{margin: '0 0 8px 0', fontSize: 24, fontWeight: 700, color: '#1e293b', display: 'flex', alignItems: 'center', gap: 12}}>
              ...
              <button type="button" style={{background: previewMode ? '#22c55e' : 'white', color: previewMode ? 'white' : '#64748b', border: '2px solid #e2e8f0', borderRadius: 8, padding: '8px 16px', fontSize: 14, fontWeight: 500, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6}} onClick={() => setPreviewMode(!previewMode)}>
                <Eye size={14} />
            ...
            <form onSubmit={save} style={{display: 'flex', flexDirection: 'column', gap: 32}}>
              ...
              <div style={{background: 'linear-gradient(135deg, #f8fafc 0%, #f1f5f9 100%)', padding: 24, borderRadius: 12, border: '2px solid #e2e8f0'}}>
                ...
                <div style={{display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 20}}>
                  <div>
                    ...
                    <select
                      style={{width: '100%', padding: '12px 16px', border: '2px solid #d1d5db', borderRadius: 8, fontSize: 14, background: 'white'}}
                      required
              ...
              <div style={{background: 'linear-gradient(135deg, #eff6ff 0%, #dbeafe 100%)', padding: 24, borderRadius: 12, border: '2px solid #3b82f6'}}>
                <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20}}>
                  ...
                  <div style={{display: 'flex', alignItems: 'center', gap: 12}}>
                    <div style={{display: 'flex', background: 'white', borderRadius: 8, border: '2px solid #93c5fd', overflow: 'hidden'}}>
                      <button type="button" onClick={() => setForm(p => ({...p, logic_operator: 'OR'}))} style={{background: form.logic_operator === 'OR' ? '#3b82f6' : 'white', color: form.logic_operator === 'OR' ? 'white' : '#3b82f6', border: 'none', padding: '8px 16px', fontSize: 14, fontWeight: 600, cursor: 'pointer'}}>
                        OR
                      </button>
                      <button type="button" onClick={() => setForm(p => ({...p, logic_operator: 'AND'}))} style={{background: form.logic_operator === 'AND' ? '#3b82f6' : 'white', color: form.logic_operator === 'AND' ? 'white' : '#3b82f6', border: 'none', padding: '8px 16px', fontSize: 14, fontWeight: 600, cursor: 'pointer'}}>
                        AND
                      </button>
                    </div>

                    <button type="button" onClick={addCondition} style={{background: 'linear-gradient(135deg, #22c55e 0%, #16a34a 100%)', color: 'white', border: 'none', borderRadius: 8, padding: '8px 16px', fontSize: 14, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6}}>
                      <Plus size={16} />
                      Add Condition
                    </button>
                  </div>
                ...
                {form.conditions.length === 0 ? (
                  <div style={{background: 'white', padding: 32, borderRadius: 8, textAlign: 'center', border: '2px dashed #93c5fd'}}>
                    <Filter size={48} color="#93c5fd" style={{marginBottom: 16}} />
                    <button type="button" onClick={addCondition} style={{background: 'linear-gradient(135deg, #3b82f6 0%, #1d4ed8 100%)', color: 'white', border: 'none', borderRadius: 8, padding: '12px 24px', fontSize: 16, fontWeight: 600, cursor: 'pointer'}}>
                  ...
                  <div style={{display: 'flex', flexDirection: 'column', gap: 12}}>
                    {form.conditions.map((condition, index) => (
                      <div key={condition.id}>
                        <div style={{background: 'white', padding: 20, borderRadius: 8, border: '2px solid #bfdbfe', position: 'relative'}}>
                          <div style={{position: 'absolute', top: -1, left: 16, background: '#3b82f6', color: 'white', padding: '4px 12px', borderRadius: '0 0 8px 8px', fontSize: 12, fontWeight: 600}}>
                            Condition {index + 1}
                          </div>

                          <div style={{display: 'grid', gridTemplateColumns: '1fr 1fr 2fr auto', gap: 16, alignItems: 'end', marginTop: 12}}>
                            <select style={{width: '100%', padding: '8px 12px', border: '1px solid #bfdbfe', borderRadius: 6, fontSize: 14, background: '#f8fafc'}} value={condition.field} onChange={(e) => updateCondition(condition.id, { field: e.target.value })}>
                              {Object.entries(CONDITION_FIELDS).map(([key, field]) => (
                                <option key={key} value={key}>{field.label}</option>
                              ))}
                            </select>

                            <select style={{width: '100%', padding: '8px 12px', border: '1px solid #bfdbfe', borderRadius: 6, fontSize: 14, background: '#f8fafc'}} value={condition.operator} onChange={(e) => updateCondition(condition.id, { operator: e.target.value })}>
                              {Object.entries(CONDITION_OPERATORS).map(([key, op]) => (
                                <option key={key} value={key}>{op.label}</option>
                              ))}
                            </select>

                            <input style={{width: '100%', padding: '8px 12px', border: '1px solid #bfdbfe', borderRadius: 6, fontSize: 14, background: 'white'}} value={condition.value} onChange={(e) => updateCondition(condition.id, { value: e.target.value })} placeholder={CONDITION_FIELDS[condition.field]?.placeholder || 'Enter value...'} />

                            <button type="button" onClick={() => removeCondition(condition.id)} style={{background: 'linear-gradient(135deg, #ef4444 0%, #dc2626 100%)', color: 'white', border: 'none', borderRadius: 6, padding: '8px', cursor: 'pointer'}}>
                              <Trash size={16} />
                            </button>
                          </div>
                        </div>
                        ...
                        {index < form.conditions.length - 1 && (
                          <div style={{textAlign: 'center', padding: '8px 0'}}>
                            <div style={{display: 'inline-flex', alignItems: 'center', gap: 8, background: form.logic_operator === 'OR' ? '#fef3c7' : '#dbeafe', color: form.logic_operator === 'OR' ? '#f59e0b' : '#3b82f6', padding: '4px 12px', borderRadius: 20, fontSize: 12, fontWeight: 700}}>
                              <ArrowDown size={14} />
                              {form.logic_operator}
                            </div>
                          </div>
              ...
              <div style={{background: 'linear-gradient(135deg, #f0fdf4 0%, #dcfce7 100%)', padding: 24, borderRadius: 12, border: '2px solid #22c55e'}}>
                ...
                <div style={{background: 'white', padding: 16, borderRadius: 8, marginBottom: 20, border: '1px solid #86efac'}}>
                  <p style={{margin: 0, fontSize: 14, color: '#166534', fontWeight: 500}}>
                    Template Variables: {'{{ application }}  {{ object_name }}  {{ node_name }}  {{ message }}'}
                  </p>
                </div>
              ...
              <div style={{display: 'flex', gap: 16, justifyContent: 'flex-end', paddingTop: 24, borderTop: '2px solid #f1f5f9'}}>
                ...
                <button type="submit" style={{background: 'linear-gradient(135deg, #7c3aed 0%, #a855f7 100%)', color: 'white', border: 'none', borderRadius: 8, padding: '12px 32px', fontSize: 16, fontWeight: 600, cursor: 'pointer', boxShadow: '0 4px 12px rgba(124, 58, 237, 0.3)', opacity: form.conditions.length === 0 ? 0.5 : 1}} disabled={form.conditions.length === 0}>
                  {editingItem ? '✓ Update Rule' : '🚀 Create Rule'}
                </button>
              </div>
            </form>
      ...
      {/* Rules List */}
      {rules.length === 0 ? (
        <div style={{background: 'linear-gradient(135deg, #fef7ff 0%, #fae8ff 100%)', border: '3px dashed #d8b4fe', borderRadius: 16, padding: 48, textAlign: 'center'}}>
          <div style={{fontSize: 48, marginBottom: 16}}>🎯</div>
          <h3 style={{fontSize: 24, fontWeight: 700, color: '#7c3aed', marginBottom: 12}}>No Rules Yet</h3>
          <p style={{fontSize: 16, color: '#a855f7', marginBottom: 24}}>Create rules to handle specific alert types with custom conditions.</p>
        </div>
      ) : (
        <div>
          ...
          <div style={{display: 'flex', flexDirection: 'column', gap: 20}}>
            {rules.map(rule => {
              ...
              return (
                <div key={rule._id} style={{background: 'white', borderRadius: 16, padding: 24, boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.1)', border: '1px solid #f1f5f9', position: 'relative', overflow: 'hidden'}}>
                  <div style={{position: 'absolute', top: 0, left: 0, right: 0, height: 4, background: rule.enabled ? 'linear-gradient(90deg, #22c55e, #10b981)' : 'linear-gradient(90deg, #64748b, #475569)'}} />
                  ...
                  <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20, marginTop: 8}}>
                    <div style={{flex: 1}}>
                      <div style={{display: 'flex', alignItems: 'center', gap: 12, marginBottom: 8, flexWrap: 'wrap'}}>
                        ...
                        <div style={{background: rule.enabled ? '#dcfce7' : '#f1f5f9', color: rule.enabled ? '#166534' : '#64748b', padding: '2px 8px', borderRadius: 4, fontSize: 11, fontWeight: 600, display: 'flex', alignItems: 'center', gap: 4}}>
                          {rule.enabled ? <CheckCircle size={12} /> : <XCircle size={12} />}
                          {rule.enabled ? 'ACTIVE' : 'INACTIVE'}
                        </div>
                        <div style={{background: '#e0f2fe', color: '#0c4a6e', padding: '2px 8px', borderRadius: 4, fontSize: 11, fontWeight: 600}}>
                          {rule.logic_operator || 'OR'} Logic
                        </div>
                        {customFieldsInRule.length > 0 && (
                          <div style={{background: '#faf5ff', color: '#9333ea', padding: '2px 8px', borderRadius: 4, fontSize: 11, fontWeight: 600}}>
                            {customFieldsInRule.length} Custom Field{customFieldsInRule.length > 1 ? 's' : ''}
                          </div>
                        )}
                      </div>
                    ...
                    <div style={{display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap'}}>
                      <button onClick={() => toggle(rule._id, !rule.enabled)} style={{background: rule.enabled ? 'linear-gradient(135deg, #f59e0b 0%, #d97706 100%)' : 'linear-gradient(135deg, #22c55e 0%, #16a34a 100%)', color: 'white', border: 'none', borderRadius: 8, padding: '6px 12px', fontSize: 12, fontWeight: 500, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4}}>
                        {rule.enabled ? <ToggleRight size={14} /> : <ToggleLeft size={14} />}
                        {rule.enabled ? 'Disable' : 'Enable'}
                      </button>
                      <button onClick={() => startEdit(rule)} style={{background: 'linear-gradient(135deg, #3b82f6 0%, #1d4ed8 100%)', color: 'white', border: 'none', borderRadius: 8, padding: '6px 12px', fontSize: 12, fontWeight: 500, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4}}>
                        <Edit size={12} />
                        Edit
                      </button>
                      <button onClick={() => del(rule._id)} style={{background: 'linear-gradient(135deg, #ef4444 0%, #dc2626 100%)', color: 'white', border: 'none', borderRadius: 8, padding: '6px 12px', fontSize: 12, fontWeight: 500, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4}}>
                        <Trash2 size={12} />
                        Delete
                      </button>
                    </div>
                      ...
                      {Object.entries(rule.conditions || {}).map(([key, value]) => {
                        if (Array.isArray(value)) {
                          return value.map((v, idx) => (
                            <div key={`${key}-${idx}`} style={{background: 'white', padding: 8, borderRadius: 4, border: '1px solid #93c5fd', fontSize: 12}}>
                              <span style={{fontWeight: 600, color: '#1e40af'}}>{key.replace(/_/g, ' ')}:</span>
                              <span style={{marginLeft: 8, fontFamily: 'monospace', color: '#3730a3'}}>"{v}"</span>
                            </div>
                          ));
                        } else if (value && typeof value === 'string') {
                          return (
                            <div key={key} style={{background: 'white', padding: 8, borderRadius: 4, border: '1px solid #93c5fd', fontSize: 12}}>
                              <span style={{fontWeight: 600, color: '#1e40af'}}>{key.replace(/_/g, ' ')}:</span>
                              <span style={{marginLeft: 8, fontFamily: 'monospace', color: '#3730a3'}}>
                                {key.includes('regex') ? `/${value}/` : `"${value}"`}
                              </span>
                            </div>
                          );
                        }
                  ...
                  {rule.incident_overrides && Object.keys(rule.incident_overrides).length > 0 && (
                    <div style={{background: 'linear-gradient(135deg, #f0fdf4 0%, #dcfce7 100%)', padding: 16, borderRadius: 8, marginBottom: 16, border: '2px solid #22c55e'}}>
                      ...
                      <div style={{display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: 8}}>
                        {Object.entries(rule.incident_overrides).filter(([k]) => k !== 'description').map(([k, v]) => (
                          <div key={k} style={{
                            background: k === 'u_system_failure' ? '#fef2f2' : 
                                       customFieldsInRule.includes(k) ? '#faf5ff' : 'white',
                            padding: 8,
                            borderRadius: 4,
                            border: k === 'u_system_failure' ? '1px solid #fecaca' : 
                                   customFieldsInRule.includes(k) ? '1px solid #e9d5ff' : '1px solid #86efac',
                            fontSize: 12
                          }}>
                            <span style={{
                              fontWeight: 600, 
                              color: k === 'u_system_failure' ? '#dc2626' : 
                                    customFieldsInRule.includes(k) ? '#9333ea' : '#166534'
                            }}>
                              {k.replace(/_/g, ' ')}:
                            </span>
                            <div style={{
                              marginTop: 4, 
                              color: k === 'u_system_failure' ? '#dc2626' : 
                                    customFieldsInRule.includes(k) ? '#6b21a8' : '#15803d',
                              fontWeight: k === 'u_system_failure' ? 600 : 'normal'
                            }}>
                              {k === 'u_system_failure' ? (v ? 'YES' : 'NO') : String(v)}
                            </div>
                          </div>
                        ))}
                      </div>
                      ...
                      {rule.incident_overrides.description && (
                        <div style={{marginTop: 12}}>
                          <div style={{fontSize: 12, fontWeight: 600, color: '#166534', marginBottom: 4}}>Description template:</div>
                          <div style={{background: 'white', border: '1px solid #86efac', borderRadius: 4, padding: 10, whiteSpace: 'pre-wrap', maxHeight: 120, overflow: 'auto', fontSize: 11, color: '#15803d'}}>
                            {rule.incident_overrides.description}
                          </div>
                        </div>
                      )}
                  ...
                  {/* Meta info */}
                  {rule.created_at && (
                    <div style={{paddingTop: 16, borderTop: '1px solid #f1f5f9', fontSize: 12, color: '#94a3b8'}}>
