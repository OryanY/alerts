...
class IncidentService {
    ...
    async createServiceNowAlert(alertData) {
        ...
        if (!systemMapping) {
            ...
            throw new Error(`No system mapping found for application: ${application}. Available: ${allMappings.map(m => m.grafana_names.map(p => p.value).join(', ')).join(' | ')}`);
        }
        ...
        if (user) alertPayload.caller_id = user;
        if (prevented && incident_sys_id) alertPayload.parent_incident = incident_sys_id;

        const serviceNowResult = await this.serviceNowClient.createIncident(alertPayload);
        return { alertPayload, serviceNowResult, mapping_used: systemMapping._id };
    }
